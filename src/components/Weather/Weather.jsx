...
export const Weather = () => {
    ...
    return (
        <div className='weather__container'>
            ...
            {weather && <div className="weather__city">
                <h3 className="weather__text">{`${weather.city.name}(${weather.city.country})`}</h3>
                <p className="weather__text">
                    <span className="description__text">Sunrise: </span>{getTimefromTimestamp(weather.city.sunrise)}
                </p>
                <p className="weather__text">
                    <span className="description__text">Sunset: </span>{getTimefromTimestamp(weather.city.sunset)}
                </p>
            </div>}
            <ul className='weather__list'>
                {filtered && Object.keys(filtered).map((key, index) => {
                    return(
